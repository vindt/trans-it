...
function showFloatingIcon(rect) {
  removeFloatingElements(); // Clear anything previous

  const iconButton = document.createElement('div');
  iconButton.id = 'translation-icon';

  const iconImg = document.createElement('img');
  iconImg.src = browser.runtime.getURL('images/icon48.png');
  ...
  // --- THE FIX IS HERE ---
  // We now calculate the 'top' position to be below the selection.
  let top = rect.bottom + 10; // 10px below the selection's bottom edge
  let left = rect.left + (rect.width / 2) - (iconButton.offsetWidth / 2);
...
function handleIconClick() {
  const iconButton = document.getElementById('translation-icon');
  ...
  `;
  // Disable the click to prevent multiple requests
...
// Shows the final tooltip with the results
function showTooltip(text, rect) {
  removeFloatingElements(); // Clear the icon/loader

  const tooltip = document.createElement('div');
  tooltip.id = 'translation-tooltip';
  const content = document.createElement('div');
  content.className = 'translation-tooltip-content';
...
// Removes both the icon and the tooltip, and any listeners
function removeFloatingElements() {
  document.getElementById('translation-icon')?.remove();
  document.getElementById('translation-tooltip')?.remove();
  document.removeEventListener('mousedown', handleClickOutside, true);
}

// Handles clicking outside to close the tooltip
...
function loadTooltipStyles() {
    if (document.getElementById('translation-tooltip-styles')) return;
    const link = document.createElement('link');
    link.id = 'translation-tooltip-styles';
    link.rel = 'stylesheet';
    link.type = 'text/css';
    link.href = browser.runtime.getURL('web/tooltip.css');
    document.head.appendChild(link);
}

function initializeMyExtension() {
  if (!viewerContainer) {
    setTimeout(initializeMyExtension, 100);
    return;
  }
  loadTooltipStyles();
  ...
  viewerContainer.addEventListener('mouseup', (event) => {
    ...
    setTimeout(() => {
        ...
        const clickedOnOurUI = event.target.closest('#translation-icon, #translation-tooltip');

        if (selection && !selection.isCollapsed && !clickedOnOurUI) {
            // A selection was made, so store its details and show the icon.
            lastSelectedText = selection.toString();
            const range = selection.getRangeAt(0);
            lastSelectionRect = range.getBoundingClientRect();
            showFloatingIcon(lastSelectionRect);
        } else if (!clickedOnOurUI) {
  ...
  document.addEventListener('keyup', (event) => {
    if (event.key === 'Shift' && lastSelectedText) {
      // Check if the icon is currently visible. This means a selection is active.
      const iconButton = document.getElementById('translation-icon');
      if (iconButton) {
        handleIconClick();
      }
  ...
  // This listener receives the processed result from background.js
  browser.runtime.onMessage.addListener((request, sender, sendResponse) => {
    if (request.action === "displayPDFTranslation") {
      if (lastSelectionRect) {
        showTooltip(request.translatedText, lastSelectionRect);
      }
    }
  });
}
